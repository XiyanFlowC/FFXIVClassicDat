    ...
    namespace xml
    {
        ...
        */
        ...
        inline XmlNodeT XmlParser<XmlNodeT, Ch>::Parse(const std::basic_string<Ch> &xml)
        {
            error = "";
            index = 0;
            ProcessPreContents(xml);
            return ParseNode(xml);
        }

        template<typename XmlNodeT, typename Ch>
        void XmlParser<XmlNodeT, Ch>::ProcessPreContents(const std::basic_string<Ch> &xml)
        {
            // 寻找第一个字符
            index = xml.find_first_not_of(EMPTY_CHARS, index);

            if (xml[index] != '<')
                throw xybase::InvalidParameterException(L"xml", L"Ill-formed XML File", 10500);
            
            while ((1))
            {
                // 顶层注释
                if (xml[index + 1] == '!' && xml[index + 2] == '-' && xml[index + 3] == '-')
                {
                ...
                * DTD 等，不支持
                ...
                {
                    index = xml.find(xybase::string::to_enc<Ch, char>("?>"), index + 2) + 2;
                    index = xml.find_first_not_of(EMPTY_CHARS, index);
                }
                else break;
            }
        ...
        template<typename XmlNodeT, typename Ch>
        std::basic_string<Ch> XmlParser<XmlNodeT, Ch>::ProcessString(const std::basic_string_view<Ch> str)
        {
            int cur = 0;
            xybase::StringBuilder<Ch> sb;
            while (cur < str.length())
            {
                if (str[cur] != '&') sb += str[cur];
                else
                {
                    // 处理转义序列
        ...
        XmlNodeT XmlParser<XmlNodeT, Ch>::ParseNode(const std::basic_string<Ch> &xml)
        {
            ...
            if (xml[index] == '<')
            {
                ...
                while (index < xml.length())
                {
                    ...
                    std::basic_string<Ch> attrValue = ProcessString(xml.substr(attrValueStart, attrValueEnd - attrValueStart));
                ...
                while (index < xml.size())
                {
                    ...
                    if (xml[index] == '<' && xml[index + 1] != '!')
                    {
                        ...
                        if (xml[index + 1] == '/')
                        {
                            ...
                            {
                                error += "Near " + xybase::string::to_string(xml.substr(index, 32)) + ": unpairing with open tag " + xybase::string::to_string(node.GetName()) + ", ignored.\n";
                                index = xml.find('>', index);
                                continue;
                            }
                            index = xml.find('>', index) + 1;
                        ...
                        while (true)
                        {
                            ...
                            else if (xml[index] == '<')
                            {
                                ...
                                else
                                {
                                    ...
                                    if (sb.Length())
                                        node.AddText(xybase::string::to_utf16(sb.ToString()));
                                    // 中断文本处理，本节点的子节点循环会处理标签的开启（子节点处理
                                    break;
                                }
                            }
                            else
